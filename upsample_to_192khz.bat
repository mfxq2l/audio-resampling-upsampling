@echo off
chcp 65001 >nul
title Audio Upsampling Tool - 192kHz FLAC 24bit

setlocal enabledelayedexpansion

echo ============================================================
echo    🎵 Audio Upsampling Tool (192kHz FLAC 24bit)
echo ============================================================
echo.

set "workdir=%~dp0"
set "outputdir=%workdir%upsampled_192"
...
for %%f in (*.flac *.wav *.mp3 *.m4a *.aac) do (
    ...
    ffmpeg -i "%%f" -af aresample=resampler=soxr:precision=33 -ar 192000 -c:a flac -sample_fmt s32 -y -hide_banner -loglevel error "%outputdir%\%%~nf.flac"
    if errorlevel 1 (
        echo     ❌ Failed
    ) else (
        echo     ✅ Done
